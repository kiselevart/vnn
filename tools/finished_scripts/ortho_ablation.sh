#!/bin/bash
# Orthogonal basis ablation — Legendre, Chebyshev, Hermite fusion models.
# All 3 splits of UCF101 and HMDB51.
#
# Models: lvn_legendre_fusion, lvn_chebyshev_fusion, lvn_hermite_fusion
#   --n_lag 4 : 4 basis functions per temporal kernel dimension
# Fusion: additive (cat(rgb, flow) → 192ch)
# Basis LR: 3× multiplier on .coeff params (default laguerre_lr_mult=3.0)
#
# Usage:
#   cd vnn
#   bash ortho_ablation.sh
#   bash ortho_ablation.sh 2>&1 | tee logs/ortho_ablation_terminal.log
...
LOG_DIR="./logs/ortho_ablation_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$LOG_DIR"

echo "Ortho basis ablation (additive fusion) — $(date '+%Y-%m-%d %H:%M:%S')"
echo "  Models:   ${MODELS[*]}"
echo "  n_lag_t:  $N_LAG  n_lag_s:  $N_LAG_S"
...
for MODEL in "${MODELS[@]}"; do
    ...
    for DATASET in "${DATASETS[@]}"; do
        ...
        for SPLIT in "${SPLITS[@]}"; do
            RUN_NAME="${DATASET}_${MODEL}_nt${N_LAG}_ns${N_LAG_S}_split${SPLIT}"
            run_ddp "$RUN_NAME" \
                --dataset     "$DATASET" \
                --model       "$MODEL" \
                --n_lag       "$N_LAG" \
...
for MODEL in "${MODELS[@]}"; do
    echo ""
    echo "  $MODEL (n_lag_t=$N_LAG n_lag_s=$N_LAG_S, additive):"
